...
#define PIX_SWAP(a,b) { FP_TYPE temp=(a);(a)=(b);(b)=temp; }
...
/*----------------------------------------------------------------------------
   Function :   opt_med9()
   In       :   pointer to an array of 9 FP_TYPEs
   Out      :   a FP_TYPE
   Job      :   optimized search of the median of 9 FP_TYPEs
...
/*----------------------------------------------------------------------------
   Function :   opt_med25()
   In       :   pointer to an array of 25 FP_TYPEs
   Out      :   a FP_TYPE
   Job      :   optimized search of the median of 25 FP_TYPEs
   Notice   :   in theory, cannot go faster without assumptions on the
                signal.
  				Code taken from Graphic Gems.
 ---------------------------------------------------------------------------*/

FP_TYPE opt_med25(FP_TYPE * p)
{
